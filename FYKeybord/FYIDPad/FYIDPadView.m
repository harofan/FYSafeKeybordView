...
#import "FYIDPadView.h"
#import "FYNumberPadModel.h"
@interface FYIDPadView()
...
- (void)didClickDeleteButton{
    if (self.targetTextField.text.length > 0) {
        ...
        if (_fyIDKeybordDelegate && [_fyIDKeybordDelegate respondsToSelector:@selector(clickDeleteButtonWithFYIDKeybordView:)]) {
            [self.fyIDKeybordDelegate clickDeleteButtonWithFYIDKeybordView:self];
        }
    }
}
...
- (NSArray <FYNumberPadModel *>*)p_createKeybordData{
    ...
    FYNumberPadModel *modelX = [[FYNumberPadModel alloc] initWithKeybordNumber:@"X"];
